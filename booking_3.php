...
$_SESSION['tarikh'] = $_POST['tarikh'];
$_SESSION['masa'] = $_POST['masa'];



// Get data from previous step
if (!isset($_POST['bengkelID']) || !isset($_POST['tarikh']) || !isset($_POST['masa'])) {
    echo "Maklumat tempahan tidak lengkap.";
    exit();
}
...
// Fetch list of kereta for this pelanggan
$sql = "SELECT keretaID, plate_kereta FROM kereta WHERE pelangganID = ?";
...
<head>
  <meta charset="UTF-8" />
  <title>Butiran Kereta - Tempahan Servis</title>
  ...
  <style>
    ...
    .stepper::after {
      ...
      top: 12px;
      left: 0;
      width: 50%; /* This covers up to step 3 (3/4 steps) */
      height: 2px;
    ...
    .btn-kemaskini {
      background: linear-gradient(45deg, #52b69a, #42947c);
      color: white;
      margin-top: 20px;
    }

    .btn-kemaskini:hover {
      background: linear-gradient(45deg, #42947c, #52b69a);
      transform: translateY(-2px);
      box-shadow: 0 5px 15px rgba(66, 148, 124, 0.3);
    }

    .btn-continue {
      background: linear-gradient(45deg, #5fc7a9, #44b190);
      color: white;
      margin-top: 10px;
    }

    .btn-continue:hover {
      background: linear-gradient(45deg, #44b190, #5fc7a9);
      transform: translateY(-2px);
      box-shadow: 0 5px 15px rgba(68, 177, 144, 0.3);
    }
...
<div class="step-container">
  ...
  <!-- Form: Choose Car -->
  <form action="booking_4.php" method="POST">
    <input type="hidden" name="bengkelID" value="<?= htmlspecialchars($bengkelID); ?>">
    <input type="hidden" name="tarikh" value="<?= htmlspecialchars($tarikh); ?>">
    <input type="hidden" name="masa" value="<?= htmlspecialchars($masa); ?>">

    <label for="keretaID">Pilih Kenderaan</label>
    <select id="keretaID" name="keretaID" class="form-control" required>
      <option value="" disabled selected>Pilih Kenderaan</option>
      <?php foreach ($keretaList as $k): ?>
        <option value="<?= $k['keretaID']; ?>"><?= $k['keretaID']; ?> - <?= $k['plate_kereta']; ?></option>
      <?php endforeach; ?>
    </select>

    <!-- Buttons -->
    <a href="kemas_kenderaan.php" class="btn btn-step btn-kemaskini">Kemaskini Kenderaan</a>
    <button type="submit" class="btn btn-step btn-continue">Continue</button>
  </form>
</div>
