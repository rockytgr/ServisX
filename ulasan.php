...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $ulasan = $_POST['ulasan'] ?? '';
    $ulasanID = uniqid('U'); // generate unique ID like U6432xxx
    $tarikh = date("Y-m-d H:i:s");

    $stmt = $conn->prepare("INSERT INTO ulasan (ulasanID, tempahanID, pelangganID, rating, ulasan, tarikh_ulasan) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssiss", $ulasanID, $tempahanID, $pelangganID, $rating, $ulasan, $tarikh);
    
    if ($stmt->execute()) {
        header("Location: maklumat_servis.php?success=1");
        exit();
    } else {
...
<main>
<div class="container">
    <h3>Penilaian Perkhidmatan</h3>
    <p class="text-muted">Tempahan ID: <?= htmlspecialchars($tempahanID) ?></p>
    ...
    <form method="POST">
      <input type="hidden" name="tempahanID" value="<?= htmlspecialchars($tempahanID) ?>">
      ...
      <div class="mb-4">
        ...
        <textarea class="form-control" name="ulasan" id="ulasan" rows="6" placeholder="Tulis ulasan anda..." required></textarea>
      </div>

      <div class="d-flex justify-content-between">
        <a href="maklumat_servis.php" class="btn btn-secondary">
          <i class="bi bi-arrow-left me-2"></i>Kembali
        </a>
        <button type="submit" class="btn btn-success">
          <i class="bi bi-send me-2"></i>Hantar
        </button>
      </div>
    </form>
  </div>
